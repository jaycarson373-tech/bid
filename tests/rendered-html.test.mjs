...
test("server-renders the BID market board in prelaunch without fabricated telemetry", async () => {
  ...
  assert.match(html, /Wallet connect is live/);
  assert.match(html, /Order signing stays disabled until BID market pools and the Solana program are configured/);
...
test("keeps the finished product free of starter-preview code", async () => {
  ...
  assert.match(page, /No signature requested/);
