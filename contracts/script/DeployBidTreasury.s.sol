...
contract DeployBidTreasury is Script {
    function run() external returns (BidFlywheelTreasury treasury, BidLiquidityVault liquidityVault) {
        ...
        require(deployer != address(0) && treasuryOwner != address(0), "zero owner");
        require(liquidityOperator != address(0), "zero operator");
        require(rewardsVault != address(0) && reserveVault != address(0), "zero vault");
